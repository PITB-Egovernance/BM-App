...
const pwd = ({route, navigation}) => {
  ...
  const chairmancount = () => {
    try {
      ...
      const apiUrl = `https://bm.punjab.gov.pk/api/deoview/${id}`;
      ...
      fetch(apiUrl)
        ...
        .then(data => {
          ...
          setCounts({
            regformCountdeo: data.regformCountdeo,
  ...
  const fetchCircleData = async () => {
    try {
      const id = syncStorage.get('bmuser_id');
      const apiUrl = `https://bm.punjab.gov.pk/api/deoentryshow/${id}?page=${
        page + 1
      }`;
  ...
  const fetchSecondCircleData = async () => {
    try {
      ...
      const response = await fetch(
        `https://bm.punjab.gov.pk/api/deoeverifiedshow/${id}`,
      );
  ...
  const fetchThirdCircleData = async () => {
    try {
      const id = syncStorage.get('bmuser_id');
      const response = await fetch(
        `https://bm.punjab.gov.pk/api/deocommitteeshow/${id}`,
      );
      const result = await response.json();
